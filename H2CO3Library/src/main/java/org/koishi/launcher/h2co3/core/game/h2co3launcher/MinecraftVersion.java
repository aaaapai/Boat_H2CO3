...
public class MinecraftVersion {
    ...
    public String getClassPath(H2CO3GameHelper gameHelper, boolean high, boolean isJava8) {
        ...
        for (Library lib : libraries) {
            ...
            String path = parseLibNameToPath(gameHelper, lib.name);
            cp.add(path);
        }
    ...
    public String[] getJVMArguments(H2CO3GameHelper gameHelper) {
            ...
            if (state == 0) {
                ...
                if (test.charAt(i) == '}') {
                    ...
                    String value = switch (key) {
                        ...
                        case "game_directory" -> gameHelper.getGameDirectory();
                        case "assets_root" -> gameHelper.getGameAssetsRoot();
                        ...
                        case "primary_jar_name" ->
                                gameHelper.getGameCurrentVersion() + "/" + id + ".jar";
                        case "library_directory" -> gameHelper.getGameDirectory() + "/libraries";
    ...
    public String[] getMinecraftArguments(H2CO3GameHelper gameHelper, boolean isHighVer) {
        StringBuilder test = new StringBuilder();
        if (isHighVer) {
    ...
    public List<String> getLibraries(H2CO3GameHelper gameHelper) {
        ...
        for (Library lib : this.libraries) {
            ...
            libs.add(parseLibNameToPath(gameHelper,lib.name));
        }
        return libs;
    }

    public String getSHA1(H2CO3GameHelper gameHelper, String libName) {
        if (SHAs == null) {
            SHAs = new HashMap<>();
            for (Library lib : this.libraries) {
                if (lib.name == null || lib.name.isEmpty() || lib.name.contains("net.java.jinput") || lib.name.contains("org.lwjgl") || lib.name.contains("platform")) {
                ...
                SHAs.put(parseLibNameToPath(gameHelper,lib.name), sha1);
            }
        }
        return SHAs.get(libName);
    }

    private String parseLibNameToPath(H2CO3GameHelper gameHelper,String libName) {
        String[] parts = libName.split(":");
        return String.format("%s%s%s%s%s%s%s%s%s-%s.jar", gameHelper.getGameDirectory() + File.separator + "libraries", File.separator, parts[0].replaceAll("\\.", File.separator), File.separator, parts[1], File.separator, parts[2], File.separator, parts[1], parts[2]);
    }
